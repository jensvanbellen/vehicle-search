...
def test_distance_from_dealer_city_name() -> None:
    ...
    km = distance_from_home_km(
        seller_name="Ekris Maastricht", country="NL", home_postcode="2548 AE"
    )
    assert km is not None and 150 < km < 175


def test_distance_from_postcode() -> None:
    km = distance_from_home_km(postcode="3011 AB", location="Rotterdam", home_postcode="2548 AE")
    assert km is not None and km < 40  # Rotterdam is close to The Hague


def test_distance_unknown_returns_none() -> None:
    assert distance_from_home_km(seller_name="Mystery Garage XYZ", home_postcode="2548 AE") is None


def test_german_city_resolves() -> None:
    km = distance_from_home_km(seller_name="BMW Köln", country="DE", home_postcode="2548 AE")
